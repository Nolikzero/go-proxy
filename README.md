...
- 🔒 **Security**: Basic authentication, host allowlist/blocklist, and TLS support
- 📊 **Observability**: Structured JSON logging with request/response metrics
- 🐳 **Production Ready**: Docker support, graceful shutdown, and comprehensive configuration
...
| `PROXY_AUTH_PASSWORD` | `` | Password for basic auth |
...
Use the dedicated `/healthz` endpoint for container orchestrators or load balancers:
...
2. **Host Filtering**: Use allowlist for maximum security
3. **TLS**: Enable TLS for the proxy server in production
4. **Access Control**: Run with minimal privileges
5. **Network**: Deploy in a secure network environment
